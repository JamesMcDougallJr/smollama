#!/usr/bin/env python3
"""
SH5461AS 4-Digit 7-Segment LED Display Demo — Raspberry Pi
...
class Display:
    """Drives the SH5461AS via a background multiplexing thread."""
    ...
    def __init__(self) -> None:
        ...
        for pin in SEG_PINS:
            GPIO.setup(pin, GPIO.OUT, initial=GPIO.HIGH)  # HIGH = segment off
    ...
    def _mux_loop(self) -> None:
        while self._running:
            ...
            for i, d_pin in enumerate(DIGIT_PINS):
                ...
                GPIO.output(DIGIT_PINS, GPIO.LOW)   # all digits off

                # Write segments a–g
                for bit in range(7):
                    on = bool(seg_byte & (1 << bit))
                    GPIO.output(SEG_PINS[bit], GPIO.LOW if on else GPIO.HIGH)

                # Decimal point
                GPIO.output(SEG_PINS[7], GPIO.LOW if dp else GPIO.HIGH)

                GPIO.output(d_pin, GPIO.HIGH)        # enable this digit
                time.sleep(0.001)
...
def main() -> None:
    ...
    disp = Display()
    disp.start()
    print("SH5461AS demo running — Press Ctrl+C to stop.\n")
